...
async function reserveWithdrawal({
  ...
  /*
   * Notify the Telegram withdrawal admin bot after the withdrawal
   * record and ledger reservation are safely created.
   *
   * Lazy require avoids the withdrawal.js <-> telegram_withdrawal.js
   * circular initialization problem.
   *
   * Telegram failure MUST NOT cancel or undo a valid withdrawal.
   */
  let telegramNotification = {
    sent: false,
    reason:
      "TELEGRAM_NOTIFICATION_UNAVAILABLE",
  };

  try {
    const telegramWithdrawal =
      require("./telegram_withdrawal");

    if (
      telegramWithdrawal &&
      typeof telegramWithdrawal.notifyWithdrawalCreated ===
        "function"
    ) {
      telegramNotification =
        await telegramWithdrawal.notifyWithdrawalCreated({
          id: withdrawalId,
          withdrawalId,
          userId: uid,
          amountUgx:
    ...
    console.error(
      "[withdrawal] Telegram withdrawal notification failed:",
      error?.stack || error
    );
  ...
  console.log(
    "[withdrawal] Telegram withdrawal result:",
    JSON.stringify(telegramNotification)
  );
